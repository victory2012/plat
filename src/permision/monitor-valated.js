import monitorDefaultPermion from './Monitor-permision';
...
export default function permissionFun() {
  ...
  if (role == null) {
    ...
    if (userData.type === 2 && userData.layerName === '采购企业') {
      const defaultper = monitorDefaultPermion.custormAdmin();
    ...
    if (userData.type === 3 && userData.layerName === '生产企业') {
      const defaultper = monitorDefaultPermion.productPer();
    ...
    if (userData.type === 3 && userData.layerName === '采购企业') {
      const defaultper = monitorDefaultPermion.custormPer();
